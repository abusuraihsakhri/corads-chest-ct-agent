import pytest
from corads_sentinel import GGOFeatureExtractorAgent, CORADSCategorizerAgent, CTSeverityIndexCalculatorAgent, ChestCTCoordinator, main


def test_sub_agents():
    a1 = GGOFeatureExtractorAgent()
    alerts1 = a1.evaluate({"metric_primary": 35.0})
    assert len(alerts1) == 1

    a2 = CORADSCategorizerAgent()
    alerts2 = a2.evaluate({"critical_flag": True})
    assert len(alerts2) == 1

    a3 = CTSeverityIndexCalculatorAgent()
    alerts3 = a3.evaluate({"status_text": "DISCORDANT_FINDING"})
    assert len(alerts3) == 1


def test_coordinator():
    coord = ChestCTCoordinator()
...
def test_domain_registry():
    from corads_sentinel import DomainKnowledgeRegistry
